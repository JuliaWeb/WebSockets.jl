...
startserver(s, url=FURL, port=FPORT)

sleep(2)

println("***************************************")
println("Server: $(s.server == nothing ? 
            "Nothing" : s.server)")
println("***************************************")

println("url: $(FURL):$(FPORT)")
...
sleep(1)
s = WebSockets.WSServer(
    HTTP.RequestHandlerFunction(req -> HResponse(200)),
    WebSockets.WebsocketHandler() do req, ws_serv
        while isopen(ws_serv)
            readguarded(ws_serv)
        end
    end);
startserver(s, "127.0.0.1", FPORT)
sleep(1)
...
s = WebSockets.WSServer(   HTTP.RequestHandlerFunction(req-> HTTP.Response(200)),
...
tas = @async serve(s, "127.0.0.1", FPORT)
...
sleep(1)
global s = WebSockets.WSServer(   HTTP.RequestHandlerFunction(req-> HTTP.Response(200)),
                               WebsocketHandler() do ws_serv
                                                while isopen(ws_serv)
                                                        read(ws_serv)
                                                end
                                            end);
global tas = @async serve(s, "127.0.0.1", FPORT, false)
while !istaskstarted(tas); yield(); end
sleep(3)
